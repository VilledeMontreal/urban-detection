...
from orion.client import report_objective ### to enable hyperparameter search with orion
...
def train(hyp, opt, device, tb_writer=None):
    ...
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, opt,
                                            ...
                                            world_size=opt.world_size, workers=opt.workers, worker_init_fn=_init_fn(0))
    ...
    if rank in [-1, 0]:
        ...
        testloader = create_dataloader(test_path, imgsz_test, total_batch_size, gs, opt,
                                       ...
                                       world_size=opt.world_size, workers=opt.workers, worker_init_fn=_init_fn(0))[0]  # testloader
    ...
    if rank in [-1, 0]:
        ...
        for f1, f2 in zip([wdir / 'last.pt', wdir / 'best.pt', results_file], [flast, fbest, fresults]):
            if os.path.exists(f1):
                ...
                if str(f2).endswith('.pt') and not opt.orion:  # is *.pt
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='data/hyp.scratch.yaml', help='hyperparameters path')
    ...
    parser.add_argument('--orion', action='store_true', help='to enable asha hyperparameter search')
    opt = parser.parse_args()

    # Set Distributed-Data-Parallel (DDP) variables
    ...
    if opt.resume:  # resume an interrupted run
        ...
        opt.data, opt.cfg, opt.config = check_file(opt.data), check_file(opt.cfg), check_file(opt.config)  # check files
        assert len(opt.cfg) or len(opt.weights), 'either --cfg or --weights must be specified'
        opt.img_size.extend([opt.img_size[-1]] * (2 - len(opt.img_size)))  # extend to 2 sizes (train, test)
        if opt.orion:
            log_dir = os.path.join(opt.logdir, opt.name)
        else:
            log_dir = increment_dir(Path(opt.logdir) / 'exp', opt.name)  # runs/exp1

    device = select_device(opt.device, batch_size=opt.batch_size)

    # Distributed Data Parallel (DDP) mode
    if opt.local_rank != -1:
    ...
    # Loading and Logging hyperparameters
    logger.info(opt)
    with open(opt.config) as f:
        hyp = yaml.load(f, Loader=yaml.FullLoader)  # load hyps

    # Train
    if not opt.evolve: # opt.evolve is yolov5's hyperparameter search procedure which uses a genetic algorithm
        tb_writer = None
        if opt.global_rank in [-1, 0]:
