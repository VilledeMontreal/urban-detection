...
def plot_ped_count(ped_count, img, img_w):
    # Plots pedestrian count on top right corner of image
    tl = 1 #line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    #cv2.rectangle(img, c1, c2, color, thickness=-1, lineType=cv2.LINE_AA)
    tf = 1  # font thickness
    ped_count_string = 'Pedestrian count: ' + str(ped_count)
    t_size = cv2.getTextSize(ped_count_string, 0, fontScale=tl / 2, thickness=tf)[0]
    c1 = (int(img_w-1-t_size[0]-2), int(0))
    c2 = (int(img_w-1), int(c1[1] + t_size[1] + 2))
    cv2.rectangle(img, c1, c2, [0, 0, 0], -1, cv2.LINE_AA)  # filled
    cv2.putText(img, ped_count_string, (c1[0]+1, c1[1]+1+t_size[1]), 0, tl / 2, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
